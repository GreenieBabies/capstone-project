...
// does not (and should not) interact with redux state
export function getTranscription(audio) {
  try {
    const relevant = audio.split(",")[1]
    const payload = { audio: relevant }
    const sendPost = async (audioIn) => {
      const { data } = await axios.post(`/api/speech`, audioIn)
      return data
    }
    const script = sendPost(payload)
    return script
  } catch (error) {
    console.log(error)
  }
